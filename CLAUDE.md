...
- **`server.py`** — HTTP server (`POST /increment`, `POST /set`, `GET /` and `GET /healthz`), stores counters in JSON, thread-safe with `file_lock`. Global state: `DATA_DIR`, `BUILD_NUMBERS_FILE`, `TOKENS_FILE`, `accept_unknown`, `max_body_size`, `max_projects`, `rate_limit`, `ban_duration`, `ban_permanent` — initialized via `init_data_dir()` in `main()` or by test fixtures. Concurrency: `PooledHTTPServer` runs a fixed pool of `--max-threads` daemon worker threads (default 64) fed by a bounded `queue.Queue` of the same size; the accept loop only enqueues sockets, workers pull and call `finish_request`. Excess connections receive `503` immediately (best-effort raw write under a 0.1s send timeout); `BuildNumberHandler.timeout = 3` gives Slowloris clients `408 Request Timeout`. `QuietHTTPServer` is kept as a backward-compat alias. TOCTOU-safe: `increment_build_number` returns `_IncrementResult{status, build_number}` (`ok` / `unapproved` / `limit_reached`) and `set_build_number` returns `_SetResult{status, build_number}` (`ok` / `unapproved`); approval/limit checks live inside the same `file_lock` section as the write. Tokens cache: `_tokens_cache` + `_tokens_cache_mtime` under `_tokens_cache_lock` invalidated by `os.path.getmtime(TOKENS_FILE)`; reset in `init_data_dir`. Rate limiting: per-IP sliding window with `rate_lock`, `rate_tracker`, `temp_bans`, `permanent_bans`; permanent bans persisted to `banned_ips.json`. `GET /healthz` bypasses rate limit and returns `{status, workers, queue_depth, uptime_seconds}`. Optional `--watchdog` runs a daemon thread polling `/healthz`; after `--watchdog-failures` consecutive failures it calls `os._exit(1)` so a wedged process gets restarted by its container orchestrator (default OFF). Optional token auth via `tokens.json` (mtime-cached, supports project-scoped, wildcard, and admin tokens). Token management CLI: `--add-token`, `--remove-token`, `--list-tokens`. Counter management CLI: `--set-counter --project-key X --version N` (offline, no HTTP server; uses `set_build_number(..., force_unapproved=True)`).
