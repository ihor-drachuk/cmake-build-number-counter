...
- **`server.py`** — HTTP server (`POST /increment`, `POST /set`, `GET /` and `GET /healthz`), stores counters in JSON, thread-safe with `file_lock`. Global state: `DATA_DIR`, `BUILD_NUMBERS_FILE`, `TOKENS_FILE`, `accept_unknown`, `max_body_size`, `max_projects`, `rate_limit`, `ban_duration`, `ban_permanent`, `trusted_proxies`, `real_ip_header` — initialized via `init_data_dir()` in `main()` or by test fixtures. Concurrency: `PooledHTTPServer` runs a fixed pool of `--max-threads` daemon worker threads (default 64) fed by a bounded `queue.Queue` of the same size; the accept loop only enqueues sockets, workers pull and call `finish_request`. Overflow: socket is closed immediately (no body, no I/O on accept thread). `BuildNumberHandler.timeout = 1` (per-recv) plus `max_request_seconds = 5` (wall-clock deadline armed via `threading.Timer` in `setup`) defends against Slowloris — header-drip attacks too, not just slow body. TOCTOU-safe: `increment_build_number` returns `_IncrementResult{status, build_number}` and `set_build_number` returns `_SetResult{status, build_number}`; `status` is an `enum.Enum` (`_IncrementStatus.OK/UNAPPROVED/LIMIT_REACHED`, `_SetStatus.OK/UNAPPROVED`); approval/limit checks live inside the same `file_lock` section as the write. Atomic durable writes: `save_json_file` does `flush()`+`fsync()` before `os.replace`. Tokens cache: `_tokens_cache` + `_tokens_cache_mtime` under `_tokens_cache_lock` invalidated by `os.path.getmtime(TOKENS_FILE)`; `load_tokens()` returns a defensive copy; reset in `init_data_dir`. Request origin: `resolve_origin()` (via `BuildNumberHandler.origin()`) returns frozen `_RequestOrigin{peer_ip, client_ip, forwarded}`. `forwarded` = TCP peer in a `--trusted-proxy` network AND the `--real-ip-header` (default `X-Real-IP`) is present; `client_ip` = the rightmost entry of all header lines joined if it is a valid IP, else the peer. `client_ip` keys rate limiting and bans; the access log prints `client via peer` when `forwarded`; `/healthz` is local only for a loopback peer that is not `forwarded`. `main()` rejects non-token header names and IPv6 trusted networks overlapping `::ffff:0:0/96`. Rate limiting: per-IP sliding window with `rate_lock`, `rate_tracker`, `temp_bans`, `permanent_bans`; permanent bans persisted to `banned_ips.json` under a separate `_bans_file_lock` so disk I/O never happens under `rate_lock`. Cleanup is **lazy** (no background thread): the IP's own bucket is purged inline on every `check_rate_limit`, plus an opportunistic global `cleanup_rate_data()` runs every `_RATE_SWEEP_EVERY` (256) allowed requests. `GET /healthz` returns `{status, uptime_seconds}` to public callers and adds `{workers, queue_depth}` for local callers only; local callers bypass rate limit, public callers are throttled. Optional `--watchdog` runs a daemon thread polling `/healthz` on the actual bound port (handles `--port 0`); after `--watchdog-failures` consecutive failures it calls `os._exit(1)` so a wedged process gets restarted by its container orchestrator (default OFF). Optional token auth via `tokens.json` (mtime-cached, supports project-scoped, wildcard, and admin tokens). Token management CLI: `--add-token`, `--remove-token`, `--list-tokens`. Counter management CLI: `--set-counter --project-key X --version N` (offline, no HTTP server; uses `set_build_number(..., force_unapproved=True)`).
