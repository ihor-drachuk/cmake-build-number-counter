...
- **`CMakeBuildNumber.cmake`** — provides `increment_build_number()` function. Two modes: `BUILD` (default, custom target at build time) and `CONFIGURE` (execute_process at configure time, can be called before `project()`). Parameters: `MODE`, `OUTPUT_VARIABLE`, `NO_INCREMENT`, `FORCE_VERSION`, `SERVER_URL`, `SERVER_TOKEN`, `LOCAL_FILE`, `TARGET`, `QUIET`. Generates `${CMAKE_BINARY_DIR}/cbnc-generated/cbnc-version.h` automatically. Creates `cbnc::version` INTERFACE library target (via `cbnc_version` + ALIAS) that propagates include directory and build dependency — consumers use `target_link_libraries(app PRIVATE cbnc::version)`. Configure mode auto-reconfigures on every build via a stamp file: created at configure, deleted by a custom target at build time, triggering reconfigure on next build.
